		...
		FOR(j,0,enum_ent_size)
		{
			...
			if (en == NULL)
			{
				...
				if (check_is_int(ent_code))
					int_list.push_back(
							make_pair((ll) en,
									strtoll(ent_code.c_str(), NULL, 10)));
				lout << "\"" << ent_code << "\"" << " return TOK_" << (ll) en
						<< ";" << endl;
			}
	...
	FOR(i,0,instr_size)
	{
		...
		if (type == "e_notpack")
		{
					...
					if (is_string)
					{
						...
						if (tmp == NULL)
						{
							...
							if (check_is_int(token))
								int_list.push_back(
										make_pair((ll) tmp,
												strtoll(token.c_str(), NULL,
														10)));
							lout << "\"" << token << "\"" << " return TOK_"
									<< (ll) tmp << ";" << endl;
						}
						yout << "TOK_" << (ll) tmp << " ";
			...
			if (bi != NULL)
			{
				if (bi->func_name != n)
				{
					dyout << n << " : " << bi->func_name
							...
							<< rbinary.length()
							<< ";dis_list[dis_list_cnt]=FUNC_" << bi->func_name
							<< ";++dis_list_cnt;};\n";
					string className = funcName2ClassName(bi->func_name);
					syout << n << " : " << bi->func_name
							<< "{ sim_dis_list_len[sim_dis_list_cnt]="
							<< rbinary.length()
			...
			FRA(i,off)
			{
				...
				yout << "for(i=0" << ";i<" << len[(int) (i - off.begin())]
						<< ";" << "++i)" << endl;
				...
				if (need_bfd[(int) (i - off.begin())])
				{
					...
					string bfd_name = "BFD_DUMMY_" + int2string(o) + "_"
							+ int2string(l) + "_"
							+ int2string((int) binary.length()) + "_"
							+ ((reloc_info[(int) (i - off.begin())] & 1) ?
									(string) "true" : (string) "false") + "_"
							+ int2string(
									reloc_info[(int) (i - off.begin())] >> 1);
					yout << bfd_name << ");" << endl;
					...
					instr_bfd_info.push_back(
							bfd_info(bfd_name, o, l, (int) binary.length(),
									ri >> 1, ri & 1));
				}
			}
				...
				for (int j = 0; j < (int) i->second.size(); ++j)
				{
					...
					if (bi != &emp)
					{
						...
						dyout << " {dis_list_len[dis_list_cnt]=" << name
								<< "_LEN " << " ;dis_list[dis_list_cnt]=FUNC_"
								<< bi->func_name << "; ++dis_list_cnt;}\n";
...
sim_dis_list[sim_dis_list_cnt] = NULL, ++sim_dis_list_cnt;}" << endl;
	...
	tout << "static bfd_reloc_code_real_type offset_reloc[100]["
			<< max_reloc_num << "];\n";
	...
	tout << "assert(howto);\n";
	tout
			<< "fixS * tmp=fix_new_exp(/*frag_now*/NULL,/*f-frag_now->fr_literal*/0,len,expr_list[cnt]+i,howto->pc_relative,offset_reloc[cnt][i]);\n";
	...
	tout.close();
	system(
			"cat ../src/as/tc-dummy1 ./tc-dummy2 ../src/as/tc-dummy3 > tc-dummy.c;\
indent ./tc-dummy.c");
	...
	FRA(i,bfd_list)
	{
		...
		bout << (i->instr_len >= 32 ? 2 : 1)
				<< ",			/* TODO!! size (0 = byte, 1 = short, 2 = long) */\n";
...
bfd_reloc_code_real_type code)\n{";
  ...
  return NULL;\n}\n";
	...
	rout.close();
	system(
			"cat ../src/as/reloc1 ./reloc2 ../src/as/reloc3 > reloc.c;\
indent ./reloc.c");
	...
	FOR(i,0,addr_size)
	{
		...
		name = "addr_" + name + "_" + (ta.get_pcrel() ? "true" : "false") + "_"
				+ int2string(width) + "_" + int2string(rs);
		tokout << "%type<chp> " << name << endl;
		yout << name << ": ";
		yout << "rule_int";
		yout << "{\n";
...
../src/as/coff-dummy3 \
 ../src/as/coff-dummy4 > coff-dummy.c; indent ./coff-dummy.c");
	...
	lout
			<< "[_a-zA-Z][_a-zA-Z0-9]*	{\n\
        dummy_lval.chp=newstr1(&strsta,yytext);/*TODO*/return TOK_LABEL;}"
			<< endl;
	lout << "[0-9]+	{dummy_lval.integer=atoi(dummy_text);return TOK_INT;}"
			<< endl;
	lout
			<< "\"0x\"[a-fA-F0-9]+ {dummy_lval.integer=strtol(dummy_text,NULL,16);return TOK_INT;}"
			<< endl;
...
{\n\
	...
	yout.close();
	tokout.close();
	string cmd = "cat " + token_file_name + " " + y_file_name + " > " + "_"
			+ y_file_name;
	system(cmd.c_str());
	...
	cmd =
			(string) "sed  --expression '2i #include \"sim_dis.y.h\"' --expression '2d ' "
					+ dl_file_name + (string) " > " + sl_file_name;
	system(cmd.c_str());
	return 0;
}
