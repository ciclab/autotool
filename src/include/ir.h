...
public:
	...
	void add_instruction(const string &name, string &c, string &b,
			vector<pp> &off, vector<string> &enum_var,
	...
	int GetStageVec(vector<Stage>** pStage);
	int GetRegister(int i, Register** pRegister);
	int GetPipline(int i, Pipeline** pPipeline);
};
#endif
