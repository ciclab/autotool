...
void varGen(ofstream &outh, ofstream &outc)
{
	...
	for (auto i : typeSet)
	{
		outh << "extern queue < pair< " << i << "*, " << i << " > >" << i
				<< "Que;\n";
		outc << " queue < pair< " << i << "*, " << i << " > >" << i << "Que;\n";
	}
	outh << "#endif\n";
	outc << "void varUpdate()\n";
	outc << "{\n";
...
void memGen(Ir &ir, ofstream & outh, ofstream & outc)
{
	...
	for (int i = 0; i < num; ++i)
	{
		...
		outh << "extern " << typeName << " " << name << " [ " << size << " ];"
				<< endl;
  ...
  virtual bool Do (){};\
	...
	for (int i = 0; i < instrSize; ++i)
	{
		...
		if (ruleType == "e_notpack")
		{
...
#ifdef LITTLE_END
			for (int i = 0, j = ruleRevBinary.length() - 8; i < j; i += 8, j -=
					8)
			{
				for (int k = 0; k < 8; ++k)
				{
			...
			assert(
					varOff.size() == varLen.size() && varOff.size() == varName.size());
			out << "//// " << ruleBinary << endl;
			for (int j = 0; j < (int) varOff.size(); ++j)
			{
				out << "set_val( " << varName[j] << ", tmp + "
						<< varOff[j].second << ", " << varLen[j] << ");\n";
			}
			out << "};\n";
			...
			for (auto i : doCnt)
			{
				...
				for (int j = 1; j < (int) i.ivec.size(); ++j)
				{
					...
					assert( i.ivec[j].is_vector() && i.ivec[j].ivec.size() > 0);
					...
					for (int k = 1; k < (int) vec.size(); ++k)
					{
								...
								&& t.ivec[0].str == "clock")
						{
							...
							out << " break;\n} ";
							timCnt.push_back(
									make_pair(stageName + (string) "_clock",
...
void dfsGenCCode(do_content & d, ofstream & out, int lev)
{
	...
	else if (d.is_vector())
	{
		...
		else
		{
			...
			// ( op a b )
			// ( ( a ) op ( b ) )
			else if (d.ivec[0].str == "&" || d.ivec[0].str == "|"
					|| d.ivec[0].str == "<<" || d.ivec[0].str == ">>"
					|| d.ivec[0].str == "-" || d.ivec[0].str == "+")
			{
				// TODO
			...
			{
				assert( d.ivec.size() == 2 && d.ivec[1].is_str1());
				int num = str2int(d.ivec[1].str);
				string clockName = (string) "clock" + int2str(timCnt.size());
				timCnt.push_back(make_pair(clockName, num));
				out << " if( --" << clockName << " > 0 ){\n ";
				out << " __ok" << lev << " = false;\n";
				out << " doOk = false; \n";
				out << "break;}\n";
